...
function countAllPeople() {
  // your code goes here
  return gradeTotal.houses.reduce((acc,cv)=>{
...
function nameWithS() {
  ...
  return got.houses.reduce((acc,house)=>{
    acc=acc.concat(
      house.people.map((p)=>p.name).filter((name)=>name.toLowerCase().includes("s"))
    );
    return acc;
  },[]);
}
...
function surnameWithS() {
  // your code goes here
  let allPeople=everyone();
  return allPeople.filter((name)=>name.split(" ").toLowerCase().includes("a"));
}
...
function surnameWithA() {
  ...
  return got.houses.reduce((acc,house)=>{
    acc=acc.concat(
      house.people.map((p)=>p.name).filter((name)=>name.split(" ")[1].toLowerCase().includes("s"))
    );
    return acc;
  },[]);
}
